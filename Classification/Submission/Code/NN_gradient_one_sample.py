...
def partial_derivatives(x, y, W, V, b, c):
    ...
    l = c + V @ sig(b + W @ x)
    s = np.sum(np.exp(l))
    dLdf = np.array([np.exp(l[0])/s,np.exp(l[1])/s,np.exp(l[2])/s-1,np.exp(l[3])/s])
    h = sig(b + W @ x)
    dLdc = dLdf
    dLdV = np.dot(dLdf,np.transpose(h))
    dLdb = np.multiply(sigp(b + W @ x),np.dot(np.transpose(V),dLdf))
    dLdW = np.dot(dLdb,np.transpose(x))
